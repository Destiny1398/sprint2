...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE vehicles SET vehicle_type = ?, make = ?, model = ?, year = ?, car_type = ?, fuel_type = ?, transmission = ?, airbags = ?, seats = ?, doors = ?, price = ? WHERE vehicle_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssiiidsi", $vehicle_type, $make, $model, $year, $car_type, $fuel_type, $transmission, $airbags, $seats, $doors, $price, $vehicle_id);
    ...
    if ($stmt->execute() === TRUE) {
        ...
        if ($vehicle_type === 'Car') {
            ...
            $feature_stmt->bind_param("iiiiiiiiiiiis", $abs, $rearview_camera, $traction_control, $air_conditioning, $power_windows_locks, $keyless_entry, $cruise_control, $adjustable_steering, $bluetooth, $navigation, $sunroof, $heated_seats, $vehicle_id);
            ...
            $feature_stmt = $conn->prepare($feature_sql);
            $feature_stmt->bind_param("iiiiiiiii", $air_conditioning, $four_wheel_drive, $bed_liner, $rearview_camera, $blind_spot_monitoring, $lane_departure_warning, $automatic_emergency_braking, $infotainment_system, $vehicle_id);
...
<body>
    ...
    <div class="admin-main-content">
        ...
        <div class="form-section">
            <h2>Edit Vehicle</h2>
            <form id="edit-vehicle-form" action="admin_edit_vehicle_details.php" method="POST">
